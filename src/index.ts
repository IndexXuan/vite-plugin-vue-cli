...
import cssLoaderCompatPlugin from './css-loader-compat'
...
export default function vueCli({ cssLoaderCompat = true }: { cssLoaderCompat: boolean }): Plugin[] {
  ...
  return <Plugin[]>[
    cssLoaderCompat ? cssLoaderCompatPlugin() : undefined,
    {
      name,
      enforce: 'pre',
      config(config) {
         ...
         */
        const defaultAlias = cssLoaderCompat
          ? [
              {
                find: /^~@\//,
